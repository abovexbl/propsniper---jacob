...
import propsniper as connector  # integrations/propsniper.py  # noqa: E402
...
def build_community(optimizer: dict | None, window: str, cdata: dict | None) -> dict:
    """Join personal performance (chosen window) against community consensus by market.
    `cdata` is the normalized community source from the connector (or None if unwired)."""
    if not cdata or not cdata.get("markets"):
        return {"status": "not_connected",
                "note": "Connect a PropSniper community source in integrations/sources.json.",
                "comparison": []}
...
def main():
    ...
    ap.add_argument("--bets", default=None, help="override; else integrations/sources.json, else sample")
    ap.add_argument("--bankroll", type=float, default=10000.0)
    ap.add_argument("--out-dir", default=str(REPO / "docs"))
    ap.add_argument("--publish", action="store_true")
    args = ap.parse_args()

    # Resolve data sources: CLI override > sources.json > sample defaults.
    sources = connector.load_sources()
    bets_path = args.bets or (sources.get("propsniper_bets") or {}).get("path") or "optimizer/sample_bets.csv"
    comm_path = (sources.get("propsniper_community") or {}).get("path") or "community/community_latest.json"
    eff_sources = {
        "propsniper_bets": {"path": bets_path},
        "propsniper_community": {"path": comm_path},
        "oddspapi": sources.get("oddspapi") or {"path": None},
    }

    audit = get_audit(args.configs)
    optimizer = get_optimizer(args.configs, bets_path, args.bankroll)
    default_window = (optimizer or {}).get("default_window", "30d")
    cdata = connector.normalize_community(comm_path)
    community = build_community(optimizer, default_window, cdata)

    feed = {
        "schema_version": SCHEMA_VERSION,
        "generated_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "source": {"configs": args.configs, "bets": bets_path, "community": community.get("source")},
        "data_sources": connector.sync_status(eff_sources),
    ...
    # OUTBOUND: write the action list the operator applies back in PropSniper.
    n_actions = connector.export_action_list((optimizer or {}).get("proposals", []),
                                              default_window, str(out_dir / "actions.csv"))
    ...
    ds = feed["data_sources"]
    print(f"  data_sources: bets={ds['propsniper_bets'].get('state')} community={ds['propsniper_community'].get('state')} oddspapi={ds['oddspapi'].get('state')}")
    print(f"  wrote {out_dir/'feed.json'} + actions.csv ({n_actions} actions)")
